<?php
    ...
    $telefono = isset($_POST["Tlf_empleado"]) ? $_POST["Tlf_empleado"] : '';
    ...
    if (isset($_POST["actualizar"])) {
        ...
        $query = "UPDATE empleados SET Nom_empleado=?, Ape_empleado=?, Tlf_empleado=?, DNI_empleado=?, clave_acceso=? WHERE Cod_empleado=?";
        // Preparar este query
        ...
        mysqli_stmt_bind_param($sentencia, "sssssi", $nombre, $apellido, $telefono, $dni,  $observaciones, $id);
        ...
        if ($afectado == 1) {
            echo "<script> alert('El empleado $nombre se editó correctamente :) '); location.href='empleados1.php' </script>";
        } else {
